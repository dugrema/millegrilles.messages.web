...
function ListeFichiers(props) {
    ...
    return (
        <div>
            {value.map(item=><Fichier key={item.fuuid} value={item} />)}
        </div>
    )
}
...
function Fichier(props) {
    ...
    return (
        <Row className="fichier">
            ...
            <Col className="nom">{value.nom}</Col>
            <Col className="taille" xs={3} md={2} lg={1}><FormatteurTaille value={value.tailleDechiffree}/></Col>
        </Row>
    )
}
