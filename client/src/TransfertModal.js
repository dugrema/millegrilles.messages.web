...
function EtatUpload(props) {
    ...
    const uploadsPending = uploads.filter(item=>item.etat===CONST_ETATS.ETAT_PRET)
    const uploadsSucces = uploads.filter(item=>[CONST_ETATS.ETAT_COMPLETE, CONST_ETATS.ETAT_CONFIRME].includes(item.etat))
    const uploadsErreur = uploads.filter(item=>[CONST_ETATS.ETAT_ECHEC, CONST_ETATS.ETAT_UPLOAD_INCOMPLET].includes(item.etat))
    const uploadEnCours = uploads.filter(item=>item.etat===CONST_ETATS.ETAT_UPLOADING).pop()
